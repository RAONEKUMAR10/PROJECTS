class Library:
    ...
    def borrowBook(self,name,bookname):
        if bookname not in self.books:
            print(f"{bookname} BOOK IS NOT AVAILABE EITHER TAKEN BY SOMEONE ELSE .\n")
            
        else:
            track.append({name:bookname})
            print(("BOOKISSUED : THANK YOU KEEP IT WITH CARE .\ n"))
            self.books.remove(bookname)
    
    def returnbook(self,bookname):
        print("BOOK RETURNED : THANKYOU VERY MUCH .\n")
        self.books.append(bookname)
    
    def donateBook(self ,bookname):
        print("BOOK DONATE : THANKYOU VERY MUCH .\n")
        self.book.append(bookname)

class Student():
    def requestBook(self):
        print("so you want to borrow book")
        self.book=input("Enter name of teh book : ")
        return self.book
